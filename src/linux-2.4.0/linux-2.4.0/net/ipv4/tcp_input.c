...
static __inline__ void tcp_ack_packets_out(struct sock *sk, struct tcp_opt *tp)
{
	if (tp->packets_out==0) {
		...
		struct sk_buff *skb = skb_peek(&sk->write_queue); // write queue first data
		__u32 when = tp->rto + tp->rttvar - (tcp_time_stamp - TCP_SKB_CB(skb)->when); // reset timer for first send out data. tp-> rto + tp-> rttvar is new rto. tcp_time_stamp - skb->when is first data wait retransmit data.
