...
#define SYSCTL_FLAG_SACK	0x4
		...
		*(((__u16 *)th) + 6)	= htons(((tcp_header_size >> 2) << 12) | tcb->flags);
		...
		err = tp->af_specific->queue_xmit(skb); // send data to ip layer...
		if (err <= 0)
			return err;
...
void tcp_send_skb(struct sock *sk, struct sk_buff *skb, int force_queue, unsigned cur_mss)
{
	...
	if (!force_queue && tp->send_head == NULL && tcp_snd_test(tp, skb, cur_mss, tp->nonagle)) { // tcp_snd_test check windows size and cwnd. whether can send data...
		/* Send it out now. */
