...
static int inet_create(struct socket *sock, int protocol)
{
	...
	case SOCK_STREAM:
		...
		prot = &tcp_prot; // bind tcp proto callback function. ex connect, read, write
		sock->ops = &inet_stream_ops; // what difference between proto and proto_ops?
		break;
	case SOCK_SEQPACKET:
...
{
	DECLARE_WAITQUEUE(wait, current); // 猜测将当前线程绑定到wait中

	__set_current_state(TASK_INTERRUPTIBLE);
	add_wait_queue(sk->sleep, &wait); // 将wait加入sleep队列中, 当有数据来时，唤醒队列中的所有线程...
	 ...
	 */
	while ((1<<sk->state)&(TCPF_SYN_SENT|TCPF_SYN_RECV)) { // wait until connect success...
		release_sock(sk);
		timeo = schedule_timeout(timeo);
		lock_sock(sk);
		if (signal_pending(current) || !timeo) // wait timeout
			break;
