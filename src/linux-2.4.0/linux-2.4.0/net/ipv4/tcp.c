		...
		int len, int nonblock, int flags, int *addr_len)
{
	...
	do {
		...
		do {
			...
			offset = *seq - TCP_SKB_CB(skb)->seq; // 剩余的未处理的数据的量
			if (skb->h.th->syn) // syn represent one seq
				offset--;
			if (offset < skb->len) // skb is the last one in queue
				goto found_ok_skb;
			if (skb->h.th->fin) // recv fin request
				goto found_fin_ok;
			if (!(flags & MSG_PEEK))
				skb->used = 1;
			skb = skb->next;
...
do_prequeue:
				tcp_prequeue_process(sk); // parse data from prequeue to receive_queue
...
static int wait_for_connect(struct sock * sk, long timeo)
{
	...
	for (;;) {
		...
		if (sk->tp_pinfo.af_tcp.accept_queue == NULL)
			timeo = schedule_timeout(timeo); // wait single or timeout.
		lock_sock(sk);
		err = 0;
