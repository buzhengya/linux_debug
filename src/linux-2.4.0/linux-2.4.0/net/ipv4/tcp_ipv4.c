...
static int tcp_v4_get_port(struct sock *sk, unsigned short snum)
{
	...
	local_bh_disable();
	if (snum == 0) { // when client connect server. port is random alloc
		...
		for (tb = head->chain; tb != NULL; tb = tb->next)
			if (tb->port == snum) // find port has used by others
				break;
	}
	if (tb != NULL && tb->owners != NULL) { // port has user by others. check last port user is release this port
	...
	ret = 1;
	if (tb == NULL && // port not use before. create tcp_bind_bucket to use this port
...
static struct sock *tcp_v4_hnd_req(struct sock *sk,struct sk_buff *skb)
{
	...
	if (req)
		return tcp_check_req(sk, skb, req, prev); // open_request has recv syn and send syn and syn_ack to client. wait client syn_ack
...
int tcp_v4_do_rcv(struct sock *sk, struct sk_buff *skb)
{
	...
	if (sk->state == TCP_ESTABLISHED) { /* Fast path */
		TCP_CHECK_TIMER(sk);
		if (tcp_rcv_established(sk, skb, skb->h.th, skb->len)) // 收数据...
			goto reset;
		TCP_CHECK_TIMER(sk);
		return 0; 
	}
...
process: // sk表示已经建立的连接或者listen状态的socket...
